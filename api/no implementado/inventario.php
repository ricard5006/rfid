...
switch ($method) {
    ...
    case "GET":
        if (isset($_GET['f006_id_usuario'])) {
            $stmt = $conn->prepare("CALL sp_find_t006_inventario(?)");
            $stmt->execute([$_GET['f006_id_usuario']]);
			
            $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
			$stmt->closeCursor();
			echo json_encode($result);
        } else {
            $stmt = $conn->query("CALL sp_list_t006_inventarios()");
            $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
			$stmt->closeCursor();
			echo json_encode($result);
        }
